...
function buildTrainDayCard(day) {
  ...
  if (day.day_type !== 'rest' || day.source === 'strava') {
    ...
    bodyHtml += '<div class="train-run-badge" style="background:' + badgeBg + ';color:' + sourceColor + ';border:1px solid ' + sourceColor + '33">'
      ...
      + day.day_type.toUpperCase() + (day.run_miles > 0 ? ' · ' + day.run_miles + ' mi' : '')
      + '</div>';
  }
...
function renderDrawer(data){
  ...
  document.getElementById('drawerBody').innerHTML=`
    ...
    <div class="meta-grid">
      <div class="meta-cell"><div class="meta-label">Distance</div><div class="meta-val or">${s.miles} mi</div></div>
      <div class="meta-cell"><div class="meta-label">Avg Pace</div><div class="meta-val">${s.avg_pace}/mi</div></div>
      <div class="meta-cell"><div class="meta-label">Moving Time</div><div class="meta-val">${s.moving_time}</div></div>
      <div class="meta-cell"><div class="meta-label">Elev Gain</div><div class="meta-val gr">${s.elev_gain} ft</div></div>
    ...
    <div class="stream-charts">
      ${hasPace?`<div class="sc-panel"><div class="sc-title">Pace /mi</div><div class="sc-wrap"><canvas id="dPaceChart"></canvas></div></div>`:''}
      ${hasHR?`<div class="sc-panel"><div class="sc-title">Heart Rate (bpm)</div><div class="sc-wrap"><canvas id="dHRChart"></canvas></div></div>`:''}
      ${hasElev?`<div class="sc-panel" style="grid-column:span 2"><div class="sc-title">Elevation (ft)</div><div class="sc-wrap"><canvas id="dElevChart"></canvas></div></div>`:''}
    </div>`:''}
    ${hasSplits?`
    <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);margin-bottom:6px">Mile Splits</div>
    <div class="splits-panel"><table class="dt"><thead><tr><th>Mile</th><th>Pace /mi</th><th>HR</th><th>Elev Δ</th></tr></thead><tbody>
      ${s.splits.map(sp=>`<tr><td>${sp.split}</td><td class="tn">${sp.pace}</td><td class="tn">${sp.hr!=='—'?sp.hr+' bpm':'—'}</td><td class="tn" style="color:${sp.elev>0?'var(--green)':sp.elev<0?'var(--red)':'var(--muted)'}">${sp.elev>0?'+':''}${sp.elev} ft</td></tr>`).join('')}
    </tbody></table></div>`:''}`;
...
async function load(){
  ...
  document.getElementById('s-wk-mi').textContent=t.week.miles+' mi';
  document.getElementById('s-wk-sub').textContent=`${t.week.runs} run${t.week.runs!==1?'s':''} · ${t.week.time}`;
  document.getElementById('s-mo-mi').textContent=t.month.miles+' mi';
  document.getElementById('s-mo-sub').textContent=`${t.month.runs} runs · ${t.month.time}`;
  document.getElementById('s-yr-mi').textContent=t.year.miles+' mi';
  document.getElementById('s-yr-sub').textContent=`${t.year.runs} runs · ${t.year.time}`;
  document.getElementById('s-at-mi').textContent=t.all_miles+' mi';
...
function renderOverviewCharts(){
  ...
  d.weekly_table.forEach((w,i)=>{const tr=document.createElement('tr');const lid=w.run_ids&&w.run_ids.length>0?w.run_ids[w.run_ids.length-1]:null;if(lid){tr.className='clickable';tr.onclick=()=>openRun(lid);}tr.innerHTML=`<td>${w.week} ${i===0?'<span class="bk bkb">now</span>':''}</td><td class="tn">${w.runs}</td><td><span class="bk bkg">${w.miles} mi</span></td><td class="tn">${w.pace}</td><td class="tn">${w.time}</td><td class="tn">${w.elev>0?w.elev+' ft':'—'}</td>`;wb.appendChild(tr);});
  ...
  d.recent.forEach(r=>{
    if(isMobile){
      ...
      tr.innerHTML='<td colspan="7" style="padding:10px 12px">'
        ...
        +'<span class="bk bko">'+r.miles+' mi</span>'
        +'</div>'
      ...
      tr.className='clickable';
      tr.onclick=()=>openRun(r.id);
      tr.innerHTML='<td><span class="run-link">'+r.name+'</span></td><td class="tn">'+r.date+'</td><td><span class="bk bko">'+r.miles+'</span></td><td class="tn">'+r.pace+'</td><td class="tn">'+r.time+'</td><td class="tn">'+(r.hr!=='—'?Math.round(r.hr)+' bpm':'—')+'</td><td class="tn">'+(r.elev>0?r.elev+' ft':'—')+'</td>';
      rb.appendChild(tr);
    }
  });
}
...
async function loadPerformance(){
  ...
  const sm=d.summary;
  if(sm.curr_pace){
    document.getElementById('p-pace').textContent=pFmt(sm.curr_pace)+'/mi';
    document.getElementById('p-pace-sub').textContent=`vs ${pFmt(sm.prev_pace||0)}/mi prior 8 wks`;
  ...
  d.heatmap.forEach(cell=>{
    ...
    div.title=cell.miles>0?`${cell.date}: ${cell.miles} mi`:cell.date;
  ...
  panel.innerHTML = `
    <div class="fuel-detail-header">
      <div>
        ...
        <div style="font-size:.7rem;color:var(--muted)">${day.display_date}${day.run_name ? ' · ' + day.run_name : ''}${day.run_miles > 0 ? ' · ' + day.run_miles + ' mi' : ''}</div>
      </div>
